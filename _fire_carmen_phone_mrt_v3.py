#!/usr/bin/env python3
"""Fire Carmen phone-MRT v3 — Grace puzzled, Carmen pacing, softened line.
...
1, 7s, Insert, Static, Tight cutaway — We see MS, Handheld, Handheld follow — MS Grace weaving and brisk-walking through the Dhoby Ghaut concourse, work bag over her shoulder, phone pressed to her ear looking ahead in a hurry. Anonymous commuters walk past her in soft focus. Carmen's voice begins in voice-over over the cut, slightly filtered through the phone line. CARMEN（电话里）：Grace 啊 —— 你听我说。 (Singapore Chinese accent), Phone-line filter + faint MRT station ambient (announcement chime distant). CARMEN（电话里）：你可以全部做完。 (Singapore Chinese accent), Escalator hum; phone-filter. CARMEN（电话里）：还有礼拜天。 Quick cutaway — commuter feet and the patterned MRT station floor, soft focus, crowd flow. SFX: Crowd footsteps; station chime; phone-filter., GRACE：怎么做。 (Singapore Chinese accent) (puzzled; eyes focused forward; jaw tight), Crowd flow; phone-filter under Grace's voice.

2, 5s, MS, Static, SPLIT-SCREEN — Grace handheld through Singapore train station crowd (left half of frame) / Carmen pacing in her apartment, mug of tea on the table behind her (right half). FIRST visual reveal of Carmen. The shot holds through Carmen's reply and Grace's response — both halves keep playing as the dialogue exchanges. Then the split-screen widens and Grace walks toward camera into the foreground, her body crossing the frame and turning the screen black., CARMEN：你可能不会喜欢这答案。 GRACE：你说。 (Singapore Chinese accent) (Carmen: eyes narrow; mouth soft. Grace: jaw sets; eyes harden on the final line), Crowd flow continues (left); quiet apartment ambient (right); piano note enters under.
...
def submit():
    ...
    print(f"Submitting Carmen phone-MRT v3 (Grace puzzled, Carmen pacing, softened line):")
...
def main():
    ...
    local_path = local / "set-05-carmen-phone-mrt-v3-puzzled-pacing-480p-15s.mp4"
    ...
    fname = "carmen-phone-mrt-v3-puzzled-pacing-480p-15s.mp4"
    ...
    Path("/tmp/carmen_phone_mrt_v3_result.json").write_text(json.dumps({
