#!/usr/bin/env python3
"""Fire Carmen apt batch B — revelation (3 shots, 12s exact).
Carmen white→black, Grace blue→white. Blocking ref image + apt image + Carmen + Grace."""
...
Blocking and character reference: see attached blocking image — Carmen and Grace seated side-by-side at the dining table, MacBook open between them, framed photo on the cream wall.

Wardrobe: Grace (on the LEFT) wears a white blouse. Carmen (on the RIGHT) wears a black sleeveless tank.

1, 4s, MS, Static, Medium shot of Carmen and Grace side-by-side. Carmen turns to face Grace directly — voice lower — this is the actual revelation., CARMEN：这才是关键 —— 不是一个助理。是好几个 —— 同时在跑。 (Singapore Chinese accent) (Carmen turns toward Grace; eyes widen slightly; Grace's eyes lift from screen to Carmen's face), Apartment ambient.

2, 5s, MS, Static, Medium shot of Carmen and Grace side-by-side at the MacBook. Carmen's right hand gestures at the open screen, walking Grace through the overnight workflow., CARMEN：我睡觉的时候,几个 agent 在跑 —— 做研究、准备几个策略方向。早上一起床 —— 一份 daily briefing 已经在桌面上等我。喝杯咖啡的时间,我就上手开工。 (Singapore Chinese accent) (Hand gestures at the screen, sweeping across the briefing as she speaks; calm, sure; Grace's eyes track Carmen's hand on the screen), Apartment ambient.

3, 3s, CU, Static, Close-up of Grace looking at Carmen's screen. Eyes widen — disbelief shading into wonder., GRACE：同时? (Singapore Chinese accent) (Eyes widen; jaw drops slightly; eyes on screen), Apartment ambient."""

REFS = [
    {"type": "video", "url": "asset://asset-20260516170559-dvf29", "label": "CARMEN"},
    {"type": "video", "url": "asset://asset-20260514120424-thhzh", "label": "GRACE"},
    {"type": "image", "url": "asset://asset-20260514140556-smw7q", "label": "CARMEN APT (still)"},
...
def submit():
    ...
    print(f"Submitting Carmen apt batch B revelation (3 shots, 12s exact):")
    print(f"  prompt: {len(PROMPT)} chars  ·  refs: {len(REFS)}")
    for r in REFS:
        print(f"    {r['label']:<22} → {r['url']} ({r['type']})")
    r = requests.post(
...
def main():
    ...
    local_path = local / "set-06-carmen-apt-batchB-revelation-12s-480p.mp4"
    ...
    fname = "carmen-apt-batchB-revelation-12s-480p.mp4"
    ...
    Path("/tmp/carmen_apt_batchB_result.json").write_text(json.dumps({
