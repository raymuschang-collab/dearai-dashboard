#!/usr/bin/env python3
"""Fire Carmen apt batch B — revelation (3 shots, 15s exact).
...
44, 4s, CU, Static, Close-up of Grace. She turns from the screen to look down at her own hands in her lap. She speaks small, scared., GRACE：Carmen —— 我不会写 prompt。我也不会 code。我是设计师。 (Singapore Chinese accent) (Eyes drop to hands; jaw tightens), Apartment ambient.

45, 7s, MS, Static, Medium shot of Carmen and Grace side-by-side. Carmen turns toward Grace, expression softens — she's been here too. The laptop is still open between them, screen glow gentle on both faces., CARMEN：我以前也不会。 ... 后来找了个 coach。Raymus —— 他和 CreativesAtWork 合作。他们专门帮 freelancer 学这些。他那个礼拜六陪我坐了一个下午。之后我才上手。 (Singapore Chinese accent) (Eyes crinkle at the corners; small smile; brief distant look on the Raymus memory; Grace's eyes lift to Carmen's face), Apartment ambient.

46, 4s, CU, Static, Close-up of Grace. The hesitation softens. She asks the question cheerfully and cheekily, eyes meeting Carmen's., GRACE：你愿意做我的 coach 吗? (Singapore Chinese accent) (Eyes widen slightly; breath held), Apartment ambient."""
...
def submit():
    ...
    print(f"Submitting Carmen apt batch D coach-offer (3 shots, 15s exact):")
...
def main():
    ...
    local_path = local / "set-06-carmen-apt-batchD-coach-15s-480p.mp4"
    ...
    fname = "carmen-apt-batchD-coach-15s-480p.mp4"
    ...
    Path("/tmp/carmen_apt_batchD_result.json").write_text(json.dumps({
