#!/usr/bin/env python3
"""Channel 8 Underwater — Shot 08 Seedance vidgen.
15s · 720p · 16:9 · Multi-shot OTS montage: emperor speaking to subjects.

Refs (all fast-forwarded to fit 15s video budget):
  - video: PALACE 3s (5x FF montage)
  - video: EMPEROR 3s (1.49x FF)
  - video: PRINCESS 3s (1.42x FF)
  - video: ELDER 3s (1.49x FF)
  - audio: SHOT8 DIALOGUE (mp3, ~5.6s)

Video budget: 3 + 3 + 3 + 3 = 12s ✓ (3s under cap)
Note: MINISTER described textually only (kneeling beside princess + elder).
"""
...
# Fast-forwarded refs (uploaded today)
PALACE_3S_REF   = "asset://asset-20260518092140-7tzqw"   # UNDERWATER PALACE 3s (5x FF)
EMPEROR_3S_REF  = "asset://asset-20260518092150-rkvfg"   # EMPEROR 3s (1.49x FF)
PRINCESS_3S_REF = "asset://asset-20260518092201-bxlnw"   # PRINCESS 3s (1.42x FF)
ELDER_3S_REF    = "asset://asset-20260518092211-8cz4x"   # ELDER 3s (1.49x FF)
...
Shot A (WS, 4s, OTS from behind PRINCESS, ELDER, MINISTER kneeling in a row): Wide shot of the underwater throne hall. In the foreground (silhouetted, soft-focus), three figures kneel in a row with backs to camera — PRINCESS on the left, ELDER in the middle, MINISTER on the right. Beyond them, on a raised dais, the EMPEROR sits on the underwater throne and addresses his subjects with regal authority. Footsoldiers in armor stand at attention behind the throne and along the columns. Bubbles drift up through the frame; light shafts pierce the water from above.

Shot B (MS, 3s, OTS from behind the throne looking out at the three kneeling subjects): Over the EMPEROR's shoulder, we see the three subjects kneeling, heads bowed, listening attentively. Their hanfu robes billow gently in the underwater current. The PRINCESS lifts her gaze for a beat — eyes respectful, attentive. The ELDER nods slowly. The MINISTER (kneeling on the right) holds his head bowed.

Shot C (Overhead high-jib MS looking straight down, 4s): God's-eye view straight down on the scene — the EMPEROR seated centered on the throne at the top of the frame, the three subjects kneeling in a horizontal row beneath him. Soldiers ring the perimeter. Schools of fish drift through the upper portion of the frame. Bubbles spiral up past camera.
...
def main():
    ...
    # BytePlus caps at 3 video refs (separate from 15s duration cap). Drop PALACE —
    # setting continuity from shot 07 holds, prioritize 3 character identity locks.
    refs = [
        ("video", EMPEROR_3S_REF,  "EMPEROR 3s (1.49x FF)"),
        ("video", PRINCESS_3S_REF, "PRINCESS 3s (1.42x FF)"),
        ("video", ELDER_3S_REF,    "ELDER 3s (1.49x FF)"),
        ("audio", SHOT8_AUDIO_REF, "SHOT8 DIALOGUE"),
    ]

    prompt = GLOBALS + "\n\n" + SHOT_PROMPT
    print(f"\nPrompt length: {len(prompt)} chars  ·  video budget: 9s (3 chars, palace dropped) ✓")
    ...
    print(f"\n  ◦ submitting shot 08 (15s · 720p · 16:9)...")
    tid = submit(content)
    print(f"    ✓ task_id: {tid}")
    result = poll(tid)
    video_url = extract_url(result)
    data = requests.get(video_url, timeout=600).content
    out_path = OUT_DIR / "shot_08_underwater_v4_3sFF_720p_15s.mp4"
