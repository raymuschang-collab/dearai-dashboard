#!/usr/bin/env python3
"""Fire Daniel home-studio scene — batch A+B combined (7 shots, 19s → 15s output).
Verbatim user prompt. Refs: Grace (5s) + Daniel v2 (7s) = 12s ✓.
Seedance will compress 19s of content into 15s output (~2.14s avg/shot)."""
...
def submit():
    ...
    print(f"Submitting Daniel home-studio batch A+B (7 shots, 19s → 15s output):")
...
def main():
    ...
    local_path = local / "set-04-daniel-studio-batchAB-480p-15s.mp4"
    ...
    fname = "daniel-studio-batchAB-7shot-480p-15s.mp4"
    ...
    Path("/tmp/daniel_studio_batchAB_result.json").write_text(json.dumps({
