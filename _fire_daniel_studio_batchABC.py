#!/usr/bin/env python3
"""Fire Daniel home-studio scene — batch A+B+C combined (9 shots, 24s → 15s output).
Heavy compression: 1.67s avg/shot. Refs: Grace (5s) + Daniel v2 (7s) = 12s ✓."""
...
Location: Grace's Home Studio — NIGHT. A modern Tiong Bahru walk-up flat home studio. Twin-monitor desk, monstera plants, terrazzo flooring, cream walls, bamboo blinds half-drawn, warm desk lamp.

1, 4s, MS, Low Angle Static, Low-angle medium shot of Grace at her twin-monitor home studio desk. Twin screens glowing, late night. Only the desk lamp and monitor glow lighting the room. Cream tee, hunched posture. Her hands move on the keyboard., (Eyes locked on screen; jaw set), Keyboard tap; faint screen hum. From behind her, coming into focus, Daniel enters from the doorway in the background holding a cup of hot tea. Faded navy shirt, glasses catching the lamp light. He approaches quietly and sets one mug of tea on Grace's desk beside her keyboard. He doesn't speak yet., (Gentle hand placement; doesn't push), Mug set down on wood. Daniel says: "又晚?" (Soft eyes; small head tilt; Singapore Chinese accent).
...
6, 2s, MCU, Static, Medium close-up of Daniel., DANIEL：报税? (Singapore Chinese accent) (Quiet voice; small head turn), Quiet ambience.

7, 2s, CU, Static, Close-up of Grace., GRACE：礼拜三。 (Singapore Chinese accent) (Eyes drop), Quiet ambience.

8, 3s, MCU, Static, Medium close-up of Daniel., DANIEL：阿嬷礼拜天。 (Singapore Chinese accent) (Soft statement, not a question; eyes on Grace), Quiet ambience.

9, 2s, CU, Static, Close-up of Grace., GRACE：(轻声) 我知道。 (Singapore Chinese accent) (Quiet; eyes still down), Quiet ambience."""
...
def submit():
    ...
    print(f"Submitting Daniel home-studio batch A+B+C (9 shots, 24s → 15s output):")
...
def main():
    ...
    local_path = local / "set-04-daniel-studio-batchABC-9shot-480p-15s.mp4"
    ...
    fname = "daniel-studio-batchABC-9shot-480p-15s.mp4"
    ...
    Path("/tmp/daniel_studio_batchABC_result.json").write_text(json.dumps({
