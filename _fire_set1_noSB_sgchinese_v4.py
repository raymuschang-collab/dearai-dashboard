...
6, 2s, CU, Static, Close-up of Grace. A small bitter smile that doesn't reach her eyes — silent recognition of the truth Carmen just named. (Smile doesn't reach eyes; eyelids drop slightly), Café ambient."""
...
def submit():
    ...
    print(f"Submitting set 1 v4 (re-roll) — shot 2 6s no-ponytail, shot 6 2s silent reaction:")
...
def main():
    ...
    local_path = local / "set-01-noSB-sgchinese-v4-480p-15s.mp4"
    ...
    fname = "set-01-noSB-sgchinese-v4-480p-15s.mp4"
    ...
    Path("/tmp/set1_noSB_v4_result.json").write_text(json.dumps({
