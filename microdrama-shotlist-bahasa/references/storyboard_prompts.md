...
| C | Bahasa Prompt | Auto-formula. `=GOOGLETRANSLATE(B{r},"en","id")` — Indonesian for SEA team. Hidden by default. **Required in this Bahasa variant.** |
...
For an episode with 70 atomized shots, that's 70 per-shot folders. Created up front by the build script (idempotent — reuses existing).
...
- **Cost matters.** Video gen is ~$0.30–0.70 per 4s clip vs ~$0.04 per storyboard image. 70 shots × 2 iter = 140 generations × $0.50 = ~$70/episode. Doubling iter is meaningful at this scale.
