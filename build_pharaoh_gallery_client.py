#!/usr/bin/env python3
"""
CLIENT VARIANT of build_pharaoh_gallery.py.

Differences vs the team gallery:
  - No Feedback boxes anywhere
  - Description clamped to 1 line (single sentence preview)
  - Storyboards show only iter 1 (cleaner read for client review)

Output: pharaoh_king_gallery_CLIENT.html
"""
...
OUTPUT = "/Users/raymuschang/Documents/Shotlist Workflows/pharaoh_king_gallery_CLIENT.html"
...
<div class="wrap">
  ...
  <footer>The Producer's Codex · Client preview</footer>
</div>
...
// CLIENT variant: feedback intentionally hidden.
function feedbackBlock(text) {{ return ""; }}
...
function renderStoryboards(items) {{
  return items.map(s => {{
    // CLIENT variant: iter 1 only
    const iters = (s.iters || []).filter(Boolean).slice(0, 1);
    return `<div class="card">
      ${{imgGrid(iters, "cols-1")}}
      <div class="card-body">
        <h3>Set ${{escapeHtml(s.set_num)}} — shots ${{escapeHtml(s.shot_range)}}</h3>
      </div>
    </div>`;
  }}).join("");
}}
...
function renderTab(key) {{
  ...
  const grid = (key === "locations" || key === "storyboards" || key === "characters") ? "grid wide" : "grid";
