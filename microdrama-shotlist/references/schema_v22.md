...
## Column Q — Bahasa Prompt addendum (optional)

Optional 17th column for projects where the production team works in a non-English language. Auto-translates the English Prompt (P) to the team's working language so non-English speakers can validate intent without reading every cell.

### Header and formula

| Letter | Header | Formula |
|---|---|---|
| Q | `Bahasa Prompt` (or per-market name) | `=GOOGLETRANSLATE(P{r},"en","id")` |

Locale codes for common SEA + APAC markets:

| Locale | Code | Header convention |
|---|---|---|
| Indonesian | `id` | `Bahasa Prompt` |
| Tagalog / Filipino | `tl` | `Tagalog Prompt` |
| Korean | `ko` | `Korean Prompt` |
| Thai | `th` | `Thai Prompt` |
| Vietnamese | `vi` | `Vietnamese Prompt` |
| Traditional Chinese (Taiwan) | `zh-TW` | `中文 Prompt` |
| Simplified Chinese (Mainland) | `zh-CN` | `中文 Prompt` |
| Japanese | `ja` | `Japanese Prompt` |

### Properties

- **One-way data flow.** Q reads from P. Nothing reads from Q. Adding/removing it never affects upstream.
- **Live formula, never typed text.** Same rule as column P — keep it generated so it stays in sync when feeder columns change.
- **Schema is still v2.2.** Q is an addendum, not a schema bump. The locked 16-column core is unchanged.
...
- ~80–90% of rows read cleanly in the target language
- ~10–20% have awkward camera-term anglicisms (`Insert` → `Sisipkan`, `Stick figure` → `figur tongkat`, `Dolly In` → `Dolly Masuk`)
- The source-language dialogue passes through untranslated (correct behavior)
- For shows shipping in 2+ markets, the same shotlist produces N translations by adding columns Q, R, S... with different locale codes
...
### When to skip column Q entirely

- Team is comfortable working in English
- Shotlist is for a one-off deliverable, not iterative team collaboration
- Project ships in only one market and the script is already in that language
