#!/usr/bin/env python3
"""Fire Carmen phone-call / MRT crowd / Carmen apartment reveal scene.
...
Location: Dhoby Ghaut MRT Station, Singapore.

1, 7s, Insert, Static, Tight cutaway — Carmen's voice begins in voice-over over the cut, slightly filtered through the phone line., CARMEN（电话里）：Grace 啊 —— 你听我说。 (Singapore Chinese accent), Phone-line filter + faint MRT station ambient (announcement chime distant). We see MS, Handheld follow — MS Grace weaving through the Dhoby Ghaut concourse, work bag over her shoulder, phone pressed to her ear. Anonymous commuters walk past her in soft focus. CARMEN（电话里）：你可以全部做完。 (Singapore Chinese accent), Escalator hum; phone-filter. CARMEN（电话里）：还有礼拜天。 Quick cutaway — commuter feet and the patterned MRT station floor, soft focus, crowd flow. SFX: Crowd footsteps; station chime; phone-filter., GRACE：怎么做。 (Singapore Chinese accent) (Eyes focused forward; jaw tight), Crowd flow; phone-filter under Grace's voice.

2, 5s, MS, Static, SPLIT-SCREEN — Grace handheld through Dhoby Ghaut crowd (left half of frame) / Carmen seated at her own dining table at her apartment, mug of tea in front of her (right half). FIRST visual reveal of Carmen. The shot holds through Carmen's reply and Grace's response — both halves keep playing as the dialogue exchanges., CARMEN：你不会喜欢这答案。GRACE：你说。 (Singapore Chinese accent) (Carmen: eyes narrow; mouth soft. Grace: jaw sets; eyes harden on the final line), Crowd flow continues (left); quiet apartment ambient (right); piano note enters under.

3, 3s, WS, Static, WHIP CUT to exterior — dusk, an MRT train whizzes past on elevated tracks above the HDB heartlands. Singapore neighborhood rooftops in golden hour. Transitory establishing shot — time has passed., MRT train rushing past on elevated tracks; wind."""
...
def submit():
    ...
    print(f"Submitting Carmen phone-MRT-apartment scene (3 shots, 15s):")
...
def main():
    ...
    local_path = local / "set-05-carmen-phone-mrt-apartment-480p-15s.mp4"
    ...
    fname = "carmen-phone-mrt-apartment-3shot-480p-15s.mp4"
    ...
    Path("/tmp/carmen_phone_mrt_result.json").write_text(json.dumps({
