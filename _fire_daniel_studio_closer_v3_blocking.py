...
4, 5s, CU, Static, Close-up of Grace., GRACE：我只有一个人。 (Singapore Chinese accent) (Eyes glaze; small breath), Quiet — holds on the breath for 1s."""
...
def submit():
    ...
    print(f"Submitting Daniel home-studio CLOSER v3 BLOCKING-ONLY (4 shots, 15s → 15s):")
...
def main():
    ...
    local_path = local / "set-04-daniel-studio-closer-v3-blockingonly-15s-480p.mp4"
    ...
    fname = "daniel-studio-closer-v3-blockingonly-15s-480p.mp4"
    ...
    Path("/tmp/daniel_studio_closer_v3_result.json").write_text(json.dumps({
