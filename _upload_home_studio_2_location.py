#!/usr/bin/env python3
"""Upload 4.9s trimmed home-studio gen as a Location asset on BytePlus."""
import json, os, sys
...
LOCAL_MP4 = "/Users/raymuschang/Downloads/GRACE_HOME_STUDIO_2_5s.mp4"
ASSET_NAME = "GRACE HOME STUDIO 2"
...
def main():
    ...
    refs_folder = get_or_create_subfolder(drive, SHOW_FOLDER, "location-refs-5s")
    media = MediaFileUpload(LOCAL_MP4, mimetype="video/mp4", resumable=True, chunksize=1024*1024)
    f = drive.files().create(
        body={"name": "grace-home-studio-2-5s.mp4", "parents": [refs_folder]},
    ...
    new_row = [
        ...
        "LOCATIONS",   # B: Bible Tab
        aid,           # C: Asset Code
        drive_view,    # D: Source URL
        "video",       # E: Asset Type
        "Uploaded",    # F: Status
        ts,            # G: Uploaded At
        "Set 4",       # H: First Used Shot
    ]
