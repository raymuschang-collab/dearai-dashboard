...
1, 15s, WS, Long Take, ONE LONG TAKE (~15 seconds). OPENS on a composite of five Graces in the same golden-hour studio: Grace 1 sketching at iPad on her desk, Grace 2 on a sales call at the window (gesturing, smiling), Grace 3 at the kitchen counter writing her newsletter, Grace 4 reviewing a spreadsheet with coffee in hand, Grace 5 on the floor with the dog. Camera slowly pushes toward Grace 1 at the desk. The other four Graces fade out gently one by one as the push continues. Camera lands on Grace 1's hands on her keyboard — small private smile. Camera continues forward to her MacBook screen — the Claude.ai chat interface clear on the screen with the "Evening, Grace" greeting visible. Then the camera pulls back wide to the empty golden-hour studio. Title card fades in over the room — "给一个人在打拼的你 · For the ones building alone." Fade to black. AT THE SAME TIME AT START OF LONG TAKE, GRACE（旁白）：以前啊 —— 在新加坡想自己干 —— 要养七个人。现在啊 —— 一台手提电脑就够了。这里的未来 —— 感觉轻了一点。不是对那些大公司 —— 他们本来就没事。是对我们这些 —— 以前一个人要当七个人用的小家伙。我们终于可以 —— 做回一个人。 (Singapore Chinese accent) (Grace 1: small private smile; hands relaxed on keyboard), Soft piano builds through the take, then pulls to silence on the last line."""
...
def submit():
    ...
    print(f"Submitting final long take v2 (new cropped laptop screen):")
...
def main():
    ...
    local_path = local / "set-12-final-longtake-v2-evening-grace-480p-15s.mp4"
    ...
    fname = "final-longtake-v2-evening-grace-480p-15s.mp4"
    ...
    Path("/tmp/final_longtake_v2_result.json").write_text(json.dumps({
