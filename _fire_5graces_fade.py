#!/usr/bin/env python3
"""Fire 5-Graces composite + fade-out variant — 8s, WS hold then peel."""
...
1, 8s, WS, Long Take, ONE LONG TAKE (~8 seconds). OPENS on a wide composite shot of FIVE Graces in the same golden-hour studio, all visible at once: Grace 1 sketching at the iPad on her desk, Grace 2 on a sales call at the window (gesturing, smiling), Grace 3 at the kitchen counter writing her newsletter, Grace 4 reviewing a spreadsheet with coffee in hand, Grace 5 on the floor with the dog. Each Grace is the same woman, same wardrobe, same hair, occupying the studio simultaneously. The other four Graces fade out gently one by one as the take continues — only Grace 1 (at the desk) remains by the end. AT THE SAME TIME AT START OF LONG TAKE: GRACE（旁白）：以前啊 —— 在新加坡想自己干 —— 要养七个人。现在啊 —— 一台手提电脑就够了。这里的未来 —— 感觉轻了一点。 (Singapore Chinese accent) (Each Grace: calm, in-flow, small private smile), Soft piano builds through the take; faint ambient apartment sound — Apple Pencil, distant traffic, dog tags."""
...
def submit():
    ...
    print(f"Submitting 5-Graces composite + fade-out (8s WS hold):")
...
def main():
    ...
    local_path = local / "set-12-5graces-fade-8s-480p.mp4"
    ...
    fname = "5graces-fade-8s-480p.mp4"
    ...
    Path("/tmp/5graces_fade_result.json").write_text(json.dumps({
