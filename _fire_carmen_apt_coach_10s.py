...
45, 10s, MCU, Static, Medium close-up of Carmen talking to Grace who is seated beside her. The laptop is still open between them, screen glow gentle on both faces., CARMEN：我以前也不会。 ... 后来找了个 coach。他和 CreativesAtWork 合作。他们专门帮 freelancer 学这些。他那个礼拜六陪我坐了一个下午。之后我才上手。 (Singapore Chinese accent) (Eyes crinkle at the corners; small smile; brief distant look on the memory; Grace's eyes lift to Carmen's face), Apartment ambient."""
...
def submit(duration):
    ...
    print(f"Submitting Carmen apt coach Raymus story (duration={duration}s):")
...
def main():
    ...
    local_path = local / f"set-06-carmen-apt-coach-10s-{chosen_dur}s-480p.mp4"
    ...
    fname = f"carmen-apt-coach-10s-{chosen_dur}s-480p.mp4"
