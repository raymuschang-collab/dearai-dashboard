...
4, 6s, CU, Static, Close-up of Carmen talking to Grace side-by-side. Carmen taps lightly on her MacBook — confirming, screen still visible to Grace beside her., CARMEN：现在我们喝茶。一个 agent 还在跑 —— 整理礼拜四的客户简报。 (Singapore Chinese accent) (Small confident smile; finger taps the screen briefly; mid-shot the framing leans toward Grace as Grace leans in to look at the screen), Apartment ambient."""
...
def submit(duration):
    ...
    print(f"Submitting Carmen apt tea/confirm CU (duration={duration}s):")
...
def main():
    ...
    local_path = local / f"set-06-carmen-apt-tea-confirm-{chosen_dur}s-480p.mp4"
    ...
    fname = f"carmen-apt-tea-confirm-{chosen_dur}s-480p.mp4"
