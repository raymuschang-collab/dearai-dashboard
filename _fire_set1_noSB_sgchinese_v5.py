#!/usr/bin/env python3
"""Fire set 1 v3 — café scene. Updated: shot 2 → 6s w/o ponytail, shot 6 → 2s silent reaction.
No storyboard ref. Refs: Grace + Carmen + Café image."""
...
6, 3s, CU, Static, Close-up of Grace. A small bitter smile that doesn't reach her eyes. Then she looks away — recognition of the truth Carmen just named. (Smile doesn't reach eyes; eyelids drop slightly; gaze shifts away at the end), Café ambient."""
...
def submit():
    ...
    print(f"Submitting set 1 v5 — shot 6 3s look-away:")
...
def main():
    ...
    local_path = local / "set-01-noSB-sgchinese-v5-480p-15s.mp4"
    ...
    sub = gc(videos, "set-01")
    fname = "set-01-noSB-sgchinese-v5-480p-15s.mp4"
    ...
    Path("/tmp/set1_noSB_v5_result.json").write_text(json.dumps({
